...
export function DateRangeFilter() {
  const { dateFrom, dateTo, setDateRange, availableRange, filteredTransactions } = useFinanceStore();
  ...
  return (
    <div className="rounded-2xl border border-black/10 dark:border-white/10 p-4 flex flex-wrap items-end gap-3">
      ...
      <span className="text-xs opacity-50 ml-auto">
        {filteredTransactions.length} операций {isActive ? "за выбранный период" : "всего"}
      </span>
    </div>
  );
}
