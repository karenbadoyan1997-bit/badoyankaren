...
export function answerQuestion(question: string, transactions: Transaction[]): string {
  ...
  if (/подпис|регуляр|recurring/.test(q)) {
    ...
    const lines = recurring.map((r) => `• ${r.merchant} — ${formatRub(r.amount)}/мес (${r.occurrences} раз)`);
    const total = recurring.reduce((s, r) => s + r.amount, 0);
    return `Нашёл ${recurring.length} регулярных платежей на общую сумму ${formatRub(total)}/мес:\n${lines.join("\n")}`;
  }
    ...
    if (duplicates.length > 0) {
      parts.push(
        "Возможные задвоенные списания:\n" +
          duplicates.map((d) => `• ${d.merchant}, ${formatRub(d.amount)}, ${d.dates.join(" и ")}`).join("\n")
      );
    }
    if (large.length > 0) {
      parts.push(
        "Необычно крупные операции:\n" +
          large.map((t) => `• ${t.merchant} — ${formatRub(Math.abs(t.amount))} (в ${t.deviationMultiple}x больше средней), ${t.date}`).join("\n")
      );
    }
  ...
  if (/чаще всего|больше всего трач|на что.*трач|топ категор|самая больш/.test(q)) {
    ...
    const lines = top.map((c, i) => `${i + 1}. ${c.category} — ${formatRub(c.total)} (${Math.round(c.share * 100)}% всех трат)`);
  ...
  if (/сколько.*(трач|расход)|средн.*месяц|бюджет/.test(q)) {
    ...
    return `В среднем вы тратите ${formatRub(s.avgMonthlyExpenses)} в месяц (данные за ${s.monthsCovered} мес.). Всего расходов за период: ${formatRub(
      s.totalExpenses
    )}.`;
  }
  ...
  if (/баланс|доход.*расход|остаток|накоп/.test(q)) {
    const s = summary(transactions);
    return `Доходы за период: ${formatRub(s.totalIncome)}. Расходы: ${formatRub(s.totalExpenses)}. Баланс: ${formatRub(s.net)}.`;
  }

  if (/месяц.*месяц|по месяцам|динамик/.test(q)) {
    const months = monthlyTotals(transactions);
    const lines = months.map((m) => `• ${m.month}: доход ${formatRub(m.income)}, расход ${formatRub(m.expenses)}`);
  ...
  return (
    ...
    `Кратко: за отслеживаемый период расходы составили ${formatRub(s.totalExpenses)}` +
    (top ? `, больше всего — на «${top.category}» (${formatRub(top.total)}).` : ".")
  );
}
